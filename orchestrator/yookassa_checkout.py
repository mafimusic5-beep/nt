...
from storage import create_checkout_code
...
def _update_order(order_id: str, **fields: Any) -> None:
    ...
    if not updates:
        return
    sql = "UPDATE payment_orders SET " + ", ".join(f"{key} = ?" for key, _ in updates) + " WHERE order_id = ?"
...
def mark_payment_failed(order_id: str, reason: str) -> None:
    _update_order(order_id, status="failed", email_error=reason[:500])
...
def create_yookassa_payment(order: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if vat_code:
        ...
        payload["receipt"] = {
            "customer": {"email": str(order["email"])},
            "items": [
                {
                    "description": f"Доступ к VPN-сервису Skryon, {selected['title']}, {months} мес.",
                    "quantity": "1.00",
                    "amount": amount,
                    "vat_code": vat,
    ...
    if response.status_code not in (200, 201):
        logger.error("YooKassa create payment failed: HTTP %s", response.status_code)
        raise PaymentProviderError(f"yookassa_create_http_{response.status_code}")
...
def fetch_yookassa_payment(payment_id: str) -> Dict[str, Any]:
    ...
    if response.status_code != 200:
        raise PaymentProviderError(f"yookassa_status_http_{response.status_code}")
    return response.json()


def _verify_provider_payment(order: Dict[str, Any], payment: Dict[str, Any]) -> None:
...
def _send_activation_email(order: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        port = 465
    if not password:
        _update_order(str(order["order_id"]), email_error="smtp_not_configured")
        return False
    ...
    rich = (
        ...
        f"<p style='font-size:22px;font-weight:700;letter-spacing:.04em'>{safe_code}</p>"
        f"<p>Тариф: <strong>{safe_title}</strong><br>Срок: <strong>{int(order['months'])} мес.</strong></p>"
    ...
    try:
        ...
        logger.exception("Activation email delivery failed for order %s", order["order_id"])
        _update_order(str(order["order_id"]), email_error=type(exc).__name__[:120])
        return False
...
def sync_and_fulfill(order_id: str) -> Dict[str, Any]:
    ...
    if provider_status == "canceled":
        _update_order(order_id, status="canceled")
        return get_payment_order(order_id) or order
    ...
    with _FULFILLMENT_LOCK:
        ...
        if not current.get("code"):
            ...
            _update_order(order_id, status="succeeded", paid_at=current.get("paid_at") or _now_iso())

    fulfilled = get_payment_order(order_id)
    if not fulfilled:
        raise KeyError("order_not_found")
    _send_activation_email(fulfilled)
