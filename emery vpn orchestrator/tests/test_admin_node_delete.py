...
def test_delete_node_removes_disabled_zero_slot_node(db_session):
    ...
    result = AdminNodeDeleteService(db_session).delete(node_id)

    assert result["status"] == "ok"
    assert result["detail"] == "deleted"
    assert db_session.get(VpnNode, node_id) is None
...
def test_delete_node_cleans_counted_assignments_before_delete(db_session):
    ...
    class FakeCleanup:
        def clear(self, target_node_id: int):
            row = db_session.get(VpnAssignment, assignment_id)
            row.status = "revoked"
            target = db_session.get(VpnNode, target_node_id)
            target.current_clients = 0
            db_session.commit()
            return {"cleared": 1, "failed": 0, "remaining": 0}

    result = AdminNodeDeleteService(db_session, cleanup_service=FakeCleanup()).delete(node_id)

    assert result["removed_assignments"] == 1
    assert db_session.get(VpnAssignment, assignment_id) is None
    assert db_session.get(VpnNode, node_id) is None
