...
async def reply_my_access_handler(message: Message) -> None:
    ...
    if not status.get("active"):
        await message.answer("Активной подписки нет.", reply_markup=_main_menu(message.from_user.id))
        return
    await message.answer(
        "Моя подписка:\n"
...
async def reply_support_handler(message: Message) -> None:
    ...
    if support_url:
        await message.answer(f"Поддержка: {support_url}", reply_markup=_main_menu(message.from_user.id))
        return
    await message.answer("Ссылка на поддержку не настроена.", reply_markup=_main_menu(message.from_user.id))
