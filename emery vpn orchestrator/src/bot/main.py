...
from src.bot.middlewares.admin_only import AdminOnlyMiddleware
...
async def run() -> None:
    ...
    # The gate runs before every router and silently drops all updates from
    # accounts that are not present in ADMIN_IDS. No handler, backend request,
    # FSM transition or system action is reached for an unauthorized sender.
