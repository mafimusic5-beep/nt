...
@router.post("/nodes/{node_id}/assignments/clear")
def clear_node_assignments(node_id: int, db: Session = Depends(get_db)) -> dict[str, object]:
    """Explicitly remove stale pool assignments from one VPS and reconcile slots."""
